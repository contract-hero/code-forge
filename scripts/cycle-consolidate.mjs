...
//
// Direct port of move-pr-review's consolidate.js, adapted for forge:
//   - STOP word list adjusted (drop sui/move terms; add forge terms)
//   - Default REVIEWERS=6 (one per dimension)
//   - Reads from cycles/<n>/reviewers/ instead of reviews/.raw/
//
// Usage: node cycle-consolidate.mjs <reviewers-dir>
//   <reviewers-dir> e.g. .forge/cycles/1/reviewers
//
// Reads:  <reviewers-dir>/subagent-{0..REVIEWERS}.json
// Writes: <reviewers-dir>/../_consolidated.json (one level up, in cycles/<n>/)
//
// Each cluster has:
...
const REVIEWERS_DIR = path.resolve(process.argv[2] || '.forge/cycles/1/reviewers');
const OUT = path.join(REVIEWERS_DIR, '..', '_consolidated.json');
const REVIEWERS = parseInt(process.env.REVIEWERS || '6', 10);

function loadAll() {
  const all = [];
  for (let n = 0; n <= REVIEWERS; n++) {
    const p = path.join(REVIEWERS_DIR, `subagent-${n}.json`);
    let raw;
...
// Forge-specific stopwords. Drop generic + cycle/forge meta-terms.
// Cf. move-pr-review's list which contained sui-specific tokens.
const STOP = new Set([
  'the','and','for','with','from','that','this','over','when','into','not','missing','check',
  'has','use','its','can','are','should','must','does','will','would','have',
  'forge','cycle','phase','agent','contract','review','spec',
]);
