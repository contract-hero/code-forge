...
- **Best-of-N implementer (green phase).** Main session (per `agents/implementer.md` procedure) + N=6 Sonnet workers (env `IMPLEMENTERS`) dispatched in a single parallel-Task turn. Each worker emits an independent candidate; the main session runs each against tests, picks the simplest passer, writes `synthesis-notes.md`. forge-guard rule 7 blocks serial worker dispatch. F10 (v0.4.x) extends test-file write protection to Bash. The previous shape — Opus coordinator dispatched as a subagent — was Amendment 3's silent-degradation bug; v0.4.x F8 made the implementer a procedure manual.
